            ...
            .then((data) => {
                ...
                const tableData = yarnTypes.map(type => {
                    ...
                    weeks.forEach(week => {
                        ...
                        row[`week${week}_in`] = weekData?.In || 0;
                        row[`week${week}_out`] = weekData?.Out || 0;
                    });
                    return row;
                });
                ...
                const chartData = weeks.map(week => {
                    ...
                    yarnTypes.forEach(type => {
                        ...
                        weekData[`${type}_in`] = typeData.In || 0;
                        weekData[`${type}_out`] = typeData.Out || 0;
                    });
                    return weekData;
                });
    ...
    return (
        <div className="available">
                ...
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                    ...
                    {Object.keys(yarnColors).map((type, index) => (
                        <React.Fragment key={type}>
                            <Bar dataKey={`${type}_in`} name={`${type} - In`} fill={yarnColors[type]} barSize={25} stackId={`stack${index}`} />
                            <Bar dataKey={`${type}_out`} name={`${type} - Out`} fill={yarnColors[type]} barSize={25} stackId={`stack${index}`} />
                        </React.Fragment>
                    ))}
                </BarChart>
            </ResponsiveContainer>
            ...
            <Accordion type="single" collapsible>
                <AccordionItem value="data-table" title="View Weekly Yarn Data">
                    <table>
                        <thead>
                            <tr>
                                <th>Yarn Type</th>
